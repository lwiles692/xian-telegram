...
战报始终展开胜负摘要、收益、当前气血法力和剩余精力。PvP 属于资源例外：切磋不消耗气血、法力与精力，因此 PvP 战报摘要必须明确显示“本场不消耗气血、法力与精力”；不为此重新查询或改写战斗数值。逐回合日志放入 Rich HTML 的 `<details>`；实体回退版使用 `ExpandableBlockQuote`。沿用当前日志截断上限，不在本次增加消息拆分或分页。
